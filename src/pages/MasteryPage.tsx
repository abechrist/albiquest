  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-[calc(6rem+env(safe-area-inset-bottom,24px))] text-slate-100 selection:bg-indigo-500/30">
        ...
        <div className="mx-auto flex max-w-4xl items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-slate-400">Analisis penguasaan kompetensi & titik fokus belajarmu</p>
            </div>
          </div>
          <Link
            to="/student/practice"
            className="rounded-xl border-b-2 border-emerald-700 bg-emerald-500 px-3.5 py-2 text-xs sm:text-sm font-bold text-white transition-all hover:bg-emerald-400 active:translate-y-0.5 min-h-[40px] flex items-center"
          >
            + Latihan Bebas
          </Link>
        </div>
      ...
      <main className="mx-auto max-w-4xl px-4 sm:px-6 pt-6 space-y-6">
        ...
        <div className="relative overflow-hidden rounded-3xl border border-indigo-500/30 bg-gradient-to-br from-indigo-950/80 via-slate-900 to-slate-950 p-6 sm:p-7 shadow-xl">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-5">
            <div>
              ...
              <p className="mt-1 text-xs sm:text-sm text-slate-300 max-w-md leading-relaxed">
                Dihitung secara adaptif dari penyelesaian modul, akurasi soal, dan ketuntasan di Bank Kesalahan.
              </p>
            </div>

            <div className="flex items-center gap-3">
              <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-800 bg-slate-900/80 px-4 py-3 text-center min-w-[96px]">
                <span className="text-2xl font-black text-emerald-400">
                  {subjectMasteries.flatMap((s) => s.topics).filter((t) => t.level === 'mastered').length}
                </span>
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Mahir 🟢</span>
              </div>
              <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-800 bg-slate-900/80 px-4 py-3 text-center min-w-[96px]">
                <span className="text-2xl font-black text-amber-400">
                  {subjectMasteries.flatMap((s) => s.topics).filter((t) => t.level === 'in_progress').length}
                </span>
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Proses 🟡</span>
              </div>
              <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-800 bg-slate-900/80 px-4 py-3 text-center min-w-[96px]">
                <span className="text-2xl font-black text-rose-400">
                  {weakTopics.length}
                </span>
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Perlu Latih 🔴</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {weakTopics.length > 0 && (
          <section className="rounded-3xl border border-rose-500/20 bg-rose-950/20 p-5 sm:p-6 backdrop-blur-sm">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {weakTopics.slice(0, 4).map((topic) => (
                <div
                  key={topic.id}
                  className="flex flex-col justify-between rounded-2xl border border-rose-500/30 bg-slate-900/80 p-4 sm:p-5 transition-all hover:border-rose-400/50"
                >
                  <div>
                    <div className="flex items-center justify-between">
                      <h4 className="font-bold text-white text-sm sm:text-base">{topic.title}</h4>
                      <span className="rounded-full bg-rose-500/20 px-2.5 py-0.5 text-xs font-bold text-rose-400">
                        {topic.score}%
                      </span>
                    </div>
                    <p className="mt-1.5 text-xs sm:text-sm text-rose-200/80 leading-relaxed">{topic.diagnosis}</p>
                  </div>
                  <div className="mt-4 flex items-center justify-between border-t border-slate-800/80 pt-3">
                    ...
                    <Link
                      ...
                      className="rounded-xl border-b-2 border-rose-700 bg-rose-600 px-3.5 py-2 text-xs sm:text-sm font-bold text-white shadow-sm transition-all hover:bg-rose-500 active:translate-y-0.5 min-h-[38px] flex items-center"
                    >
                      Latih Sekarang →
                    </Link>
                  </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
          <button
            ...
            className={`whitespace-nowrap rounded-xl px-4 py-2 text-xs sm:text-sm font-bold transition-all min-h-[40px] cursor-pointer ${
          ...
          {subjectMasteries.map((sub) => (
            <button
              ...
              className={`whitespace-nowrap flex items-center gap-1.5 rounded-xl px-4 py-2 text-xs sm:text-sm font-bold transition-all min-h-[40px] cursor-pointer ${
        ...
        <div className="space-y-6">
          {displayedMasteries.map((sub) => (
            <div
              key={sub.subjectId}
              className="rounded-3xl border border-slate-800 bg-slate-900/60 p-5 sm:p-6 backdrop-blur-sm"
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
                <div className="flex items-center gap-3">
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-indigo-500/10 text-2xl sm:text-3xl border border-indigo-500/20">
                    {sub.subjectEmoji}
                  </div>
                  <div>
                    <h3 className="font-bold text-white text-base sm:text-lg">{sub.subjectName}</h3>
              ...
              <div className="mt-4 grid gap-3.5 sm:grid-cols-2">
                {sub.topics.map((topic) => (
                  <div
                    key={topic.id}
                    className="rounded-2xl border border-slate-800/80 bg-slate-950/60 p-4 sm:p-5 transition-all hover:border-slate-700 space-y-3"
                  >
                    ...
                    <p className="text-xs sm:text-sm text-slate-400 line-clamp-2 leading-relaxed">{topic.diagnosis}</p>
                    ...
                    <div className="flex items-center justify-between border-t border-slate-800/50 pt-3">
                      ...
                      <Link
                        to={`/student/practice?subject=${topic.subjectId}&topic=${topic.id}`}
                        className="rounded-xl bg-indigo-600/20 border border-indigo-500/30 px-3 py-1.5 text-xs sm:text-sm font-bold text-indigo-300 hover:bg-indigo-600 hover:text-white transition-colors min-h-[36px] flex items-center"
                      >
                        Latih Topik →
                      </Link>
                    </div>
