    ...
    <div className="min-h-screen bg-slate-950 pb-24 text-slate-100 selection:bg-indigo-500/30">
      ...
      {view === 'lobby' && (
        <>
            ...
            <div className="mx-auto flex max-w-3xl items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">Latihan ujian resmi terstandar dengan sistem timer & palet soal</p>
                </div>
              </div>
            </div>
          </header>
          ...
          <main className="mx-auto max-w-3xl px-4 pt-6 space-y-6">
                ...
                {EXAM_PRESETS.map((preset) => (
                  ...
                  >
                    <div>
                      ...
                      <h4 className="mt-2 font-bold text-white text-sm">{preset.title}</h4>
                      <p className="mt-1 text-[11px] text-slate-400">{preset.subtitle}</p>
                    </div>

                    <div className="mt-4 border-t border-slate-800/80 pt-2 flex items-center justify-between text-xs">
            ...
            {/* Panduan Pengerjaan */}
            <section className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4 space-y-2">
            ...
            <div>
              <button
                ...
                className="w-full rounded-2xl border-b-4 border-indigo-700 bg-indigo-600 py-3.5 text-sm font-black text-white shadow-xl transition-all hover:bg-indigo-500 active:translate-y-1 flex items-center justify-center gap-2 cursor-pointer"
              >
              ...
              <section className="space-y-3 pt-4 border-t border-slate-800">
                ...
                <div className="space-y-2">
                  {history.slice(0, 3).map((item) => (
                    ...
                    >
                      <div>
                        <p className="text-xs font-bold text-white">{item.presetTitle}</p>
      ...
      {view === 'exam' && session && (
        <>
          ...
          <header className="sticky top-0 z-40 border-b border-slate-800/80 bg-slate-900/95 px-4 py-3 backdrop-blur-md">
            <div className="mx-auto flex max-w-3xl items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => setShowPaletteDrawer(!showPaletteDrawer)}
                  className="rounded-xl border border-slate-700 bg-slate-800 px-3 py-1.5 text-xs font-bold text-slate-200 hover:bg-slate-700 flex items-center gap-1.5"
                >
                  <span>📋</span>
                  <span>Palet Soal</span>
                  <span className="rounded-md bg-slate-700 px-1.5 py-0.5 text-[10px] text-indigo-300">
              ...
              {/* Timer Countdown */}
              <div
                className={`flex items-center gap-1.5 px-3 py-1 rounded-xl border text-xs font-black ${
              ...
              <button
                ...
                className="rounded-xl border-b-2 border-emerald-700 bg-emerald-600 px-3.5 py-1.5 text-xs font-black text-white hover:bg-emerald-500 active:translate-y-0.5 transition-all"
              >
                Kumpulkan 🏁
              </button>
            </div>
          ...
          <main className="mx-auto max-w-3xl px-4 pt-6 space-y-6">
            ...
            {showPaletteDrawer && (
              <div className="rounded-3xl border border-slate-800 bg-slate-900 p-5 shadow-2xl space-y-3">
                <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                  ...
                  <div className="flex items-center gap-3 text-[10px]">
                    <span className="flex items-center gap-1 text-emerald-400">
                      <span className="h-2 w-2 rounded-full bg-emerald-500" /> Terjawab
                    </span>
                    <span className="flex items-center gap-1 text-amber-400">
                      <span className="h-2 w-2 rounded-full bg-amber-500" /> Ragu
                    </span>
                    <span className="flex items-center gap-1 text-slate-400">
                      <span className="h-2 w-2 rounded-full bg-slate-700" /> Kosong
                    </span>
                  </div>
                </div>
              ...
              <div className="rounded-3xl border border-slate-800 bg-slate-900 p-6 space-y-5">
                <div className="flex items-center justify-between border-b border-slate-800/80 pb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="rounded-md bg-indigo-500/20 px-2 py-0.5 text-[10px] font-bold text-indigo-300 uppercase">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 rounded-xl px-3 py-1 text-xs font-bold transition-colors ${
                ...
                {/* Teks Pertanyaan */}
                <div className="rounded-2xl bg-slate-950 p-5 border border-slate-800/80">
                  ...
                  {session.questions[session.currentIndex].options.map((opt, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-3.5 w-full rounded-2xl p-4 text-left text-xs font-bold transition-all ${
                          isSelected
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-800">
                  <button
                    ...
                    className="rounded-xl border border-slate-700 bg-slate-800 px-4 py-2 text-xs font-bold text-slate-300 hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed"
                  >
                    ← Sebelumnya
                  </button>
                  ...
                  {session.currentIndex < session.questions.length - 1 ? (
                    <button
                      type="button"
                      onClick={() => setSession({ ...session, currentIndex: session.currentIndex + 1 })}
                      className="rounded-xl border-b-2 border-indigo-700 bg-indigo-600 px-4 py-2 text-xs font-bold text-white hover:bg-indigo-500 active:translate-y-0.5"
                    >
                      Selanjutnya →
                    </button>
                  ) : (
                    <button
                      type="button"
                      onClick={() => setShowSubmitModal(true)}
                      className="rounded-xl border-b-2 border-emerald-700 bg-emerald-600 px-4 py-2 text-xs font-bold text-white hover:bg-emerald-500 active:translate-y-0.5"
                    >
                      Selesai Ujian 🏁
                    </button>
                  )}
          ...
          {showSubmitModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
              <div className="w-full max-w-md rounded-3xl border border-slate-800 bg-slate-900 p-6 text-center space-y-4 shadow-2xl">
                <span className="text-4xl inline-block">🏁</span>
                <h3 className="text-lg font-black text-white">Kumpulkan Jawaban Ujian?</h3>
                ...
                {(() => {
                  ...
                  return (
                    <div className="rounded-2xl bg-slate-950 p-4 border border-slate-800 text-xs space-y-2 text-left">
                ...
                <div className="flex justify-center gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="rounded-xl border-b-2 border-emerald-700 bg-emerald-600 px-5 py-2.5 text-xs font-black text-white hover:bg-emerald-500 active:translate-y-0.5"
                  >
      ...
      {view === 'result' && result && (
        <>
          ...
          <main className="mx-auto max-w-3xl px-4 pt-6 space-y-6">
            {/* Score Hero Card */}
            <div
              className={`rounded-3xl border-2 p-6 text-center space-y-4 shadow-2xl ${
                result.passed
            ...
            >
              <span className="text-5xl inline-block">{result.passed ? '🎉' : '📚'}</span>
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-slate-400">
                  {result.presetTitle}
                </span>
              ...
              <div className="inline-flex items-center gap-2 rounded-full bg-indigo-500/20 px-4 py-1 text-xs font-bold text-indigo-300 border border-indigo-500/30">
              ...
              <div className="grid grid-cols-4 gap-2 pt-2 max-w-md mx-auto">
                <div className="rounded-xl bg-slate-950/80 p-2.5 border border-slate-800">
                  ...
                  <span className="text-sm font-black text-emerald-400">{result.correctCount}</span>
                </div>
                <div className="rounded-xl bg-slate-950/80 p-2.5 border border-slate-800">
                  <span className="text-[10px] text-slate-400 block">Salah</span>
                  <span className="text-sm font-black text-rose-400">{result.wrongCount}</span>
                </div>
                <div className="rounded-xl bg-slate-950/80 p-2.5 border border-slate-800">
                  <span className="text-[10px] text-slate-400 block">Kosong</span>
                  <span className="text-sm font-black text-slate-400">{result.unansweredCount}</span>
                </div>
                <div className="rounded-xl bg-slate-950/80 p-2.5 border border-slate-800">
                  <span className="text-[10px] text-slate-400 block">Waktu</span>
                  <span className="text-sm font-black text-amber-400">{Math.round(result.timeSpentSeconds / 60)}m</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-emerald-500/30 bg-emerald-950/20 p-4 space-y-2">
                ...
                {result.strongTopics.length > 0 ? (
                  ...
                  <p className="text-xs text-slate-400">Pertahankan latihan untuk memperbanyak topik mahir.</p>
                )}
              </div>
              ...
              <div className="rounded-2xl border border-rose-500/30 bg-rose-950/20 p-4 space-y-2">
                ...
                {result.needsPracticeTopics.length > 0 ? (
                  ...
                  <p className="text-xs text-emerald-400">Luar biasa! Tidak ada topik dengan kesalahan mayor.</p>
                )}
              </div>
            </div>
              ...
              <div className="space-y-3">
                {result.questionBreakdown.map((item, idx) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-200">{item.prompt}</p>

                    <div className="grid gap-1 text-[11px] rounded-xl bg-slate-950 p-3 border border-slate-800">
                      <div>
                    ...
                    <div className="rounded-xl bg-indigo-950/30 p-3 border border-indigo-500/20 text-[11px] text-indigo-200 leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-slate-800">
              <Link
                to="/student/mistakes"
                className="flex-1 rounded-2xl border-b-2 border-rose-700 bg-rose-600 py-3 text-center text-xs font-bold text-white shadow-md hover:bg-rose-500 transition-all"
              >
              ...
              <button
                type="button"
                onClick={() => setView('lobby')}
                className="flex-1 rounded-2xl border border-slate-700 bg-slate-800 py-3 text-center text-xs font-bold text-slate-300 hover:bg-slate-700"
              >
